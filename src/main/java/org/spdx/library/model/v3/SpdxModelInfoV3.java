 ...
 *
 */
public class SpdxModelInfoV3 implements ISpdxModelInfo {
	...
	public List<String> getSpecVersions() {
		return Arrays.asList(new String[] { "3.1.0","3.0.1" });
	}
	...
	public CoreModelObject createExternalElement(IModelStore store, String uri,
		...
		if (Objects.nonNull(typeClass)) {
			...
			return SpdxModelClassFactoryV3.getModelObject(store, uri, type, copyManager, specVersion, true, "");
		} else {
	...
	public CoreModelObject createModelObject(IModelStore modelStore,
			...
			throws InvalidSPDXAnalysisException {
		return SpdxModelClassFactoryV3.getModelObject(modelStore, objectUri, type, copyManager, specVersion, create, idPrefix);
	}
